...
void DFS_internal(
	std::unordered_map<Bag, std::optional<bool>>& hit_target,
	const std::unordered_map<Bag, BagRule>& brules,
	const Bag& current)
{
	std::optional<bool>& ht = hit_target[current];
	if (ht)
	{
		return;
	}
	...
	for (const auto& dst : brules.at(current).dst_bags)
	{
		...
		std::optional<bool>& ht2 = hit_target[dst.first];
		if (ht2.value())
		{
			ht = true;
		}
...
std::unordered_map<Bag,bool> DFS(const std::vector<BagRule>& brvec,const Bag& target)
{
	...
	std::unordered_map<Bag, bool> hit_out;
...
int main(int argc, char** argv)
{
	...
	std::cout << "Bags: " << std::count_if(counts.begin(), counts.end(), [](const std::pair<Bag, bool>& b)
		{
			return b.second;
		}
	) -1 << std::endl;
